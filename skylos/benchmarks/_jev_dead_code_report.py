...
def _paired_consistency(predictions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    original_count = sum(item["arm"] == "original" for item in predictions)
    return {
        "pair_count": len(pairs),
        "unpaired_original_count": original_count - len(pairs),
...
def _neutralization_summary(provenance: dict[str, Any]) -> dict[str, int]:
    if "neutralization_status_counts" in provenance:
        return dict(provenance["neutralization_status_counts"])
    requests = provenance.get("planned_requests", [])
    return {
        status: sum(
            item.get("neutralization", {}).get("status") == status for item in requests
        )
        for status in ("applied", "skipped", "not_applicable")
    }
    ...
    *,
    ...
    report: dict[str, Any] = {
        ...
        **provenance,
        ...
        "neutralization": _neutralization_summary(provenance),
...
def format_plan(plan: dict[str, Any]) -> str:
    ...
    if metadata["split"] is not None:
        lines.append(f"Split: {metadata['split']} ({metadata['label_state']} labels)")
    lines.extend(
        [
            ...
            "Arms: original identifiers, plus safely neutralized identifiers",
        ]
    )
    statuses = plan.get("neutralization_status_counts")
    if statuses is not None:
        lines.append(
            "Neutralized cases: "
            f"applied={statuses['applied']} "
            f"omitted={statuses['skipped'] + statuses['not_applicable']}"
        )
    return "\n".join(lines)
...
def format_report(report: dict[str, Any]) -> str:
    ...
    neutralization = report.get("neutralization", {})
    lines.extend(
        [
            "Identifier-neutralization consistency: "
            + (
                str(paired["decision_consistency"])
                if paired["pair_count"]
                else "not measured (no completed pairs)"
            )
            + f" across {paired['pair_count']} paired decisions",
            "Neutralization plan: "
            f"applied={neutralization.get('applied', 0)} "
            f"omitted={neutralization.get('skipped', 0) + neutralization.get('not_applicable', 0)} "
            f"(unsafe={neutralization.get('skipped', 0)}, "
            f"no-op={neutralization.get('not_applicable', 0)})",
