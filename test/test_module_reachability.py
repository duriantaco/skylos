...
class TestIntegrationWithAnalyzer:
    def test_unreachable_module_gets_low_confidence(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            used = pkg / "used.py"
            used.write_text('def helper():\n    return "used"\n')

            orphan = pkg / "orphan.py"
            orphan.write_text(
                "def dead_function():\n"
                '    return "nobody imports this module"\n'
                "\n"
                "def another_dead():\n"
                '    return "also dead"\n'
            )
            ...
            s = Skylos()
            result_json = s.analyze(str(root), thr=60)
            result = json.loads(result_json)
            unused_names = [f["name"] for f in result.get("unused_functions", [])]

            assert "dead_function" not in unused_names
            assert "another_dead" not in unused_names

    def test_reachable_module_unaffected(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            ...
            init = pkg / "__init__.py"
            init.write_text("from .core import used_func\n")
            ...
            core.write_text(
                ...
                '    return "used"\n'
                "\n"
                "def unused_in_reachable_module():\n"
                '    return "unused but module is reachable"\n'
            )
            ...
            result = json.loads(result_json)
            unused_names = [f["name"] for f in result.get("unused_functions", [])]
            assert "unused_in_reachable_module" in unused_names
