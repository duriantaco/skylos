...
def _scan_go_full(tmp_path: Path, code: str) -> tuple:
    try:
        resolve_go_engine_bin()
    except GoEngineError:
        pytest.skip("skylos-go engine binary not available")
...
def test_go_struct_tag_secret_key_is_not_hardcoded_secret(tmp_path):
    findings = _scan_go(
        tmp_path,
        """package main
...
def test_go_generated_file_skips_quality_findings(tmp_path):
    out = _scan_go_full(
        tmp_path,
        "\n".join(
            [
                ...
                "",
                "func generated() {",
                *[f"    value{i} := {i}; _ = value{i}" for i in range(60)],
                "}",
            ]
        ),
    )
...
def test_go_interface_methods_with_multiple_implementers_are_live(tmp_path):
    (tmp_path / "go.mod").write_text(
        "module example.com/demo\n\ngo 1.22\n",
        encoding="utf-8",
    )
...
func newDroplets() refresher { return &droplets{} }
func newDatabases() refresher { return &databases{} }

func (d *droplets) refresh() error { return nil }
func (d *databases) refresh() error { return nil }

func main(){ _, _ = newDroplets(), newDatabases() }
""",
        encoding="utf-8",
    )
    clear_go_cache()

    result = __import__("json").loads(analyze(str(tmp_path), conf=0, grep_verify=False))

    unused = {item["full_name"] for item in result.get("unused_functions", [])}
    assert "droplets.refresh" not in unused
    assert "databases.refresh" not in unused


def test_go_cumulative_flag_parser_hook_is_live(tmp_path):
    (tmp_path / "go.mod").write_text(
        "module example.com/demo\n\ngo 1.22\n",
        encoding="utf-8",
    )
...
func newParser() any { return &parser{} }

func (*parser) IsCumulative() bool { return true }

func main(){ _ = newParser() }
""",
        encoding="utf-8",
    )
    clear_go_cache()
    ...
    unused = {item["full_name"] for item in result.get("unused_functions", [])}
    assert "parser.IsCumulative" not in unused


def test_go_unsafe_package_usage_reports_once_per_line(tmp_path):
    findings = _scan_go(
        tmp_path,
        """package main
...
""",
    ...
    unsafe_findings = [
        finding for finding in findings if finding["rule_id"] == "SKY-G206"
    ]
