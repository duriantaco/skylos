...
CHALLENGE_PATH = REPO_ROOT / "benchmarks" / "dead_code" / "jev_challenge_manifest.json"
...
def _response(batch, choice="retained"):
    return {
        "model": JEV_MODEL,
        "answers": {
            question.question_id: _choice_answer(choice) for question in batch.questions
        },
        "usage": {"input_tokens": 100, "output_tokens": 10},
    }


def test_checked_in_ground_truth_builds_blind_plan_without_duplicate_arms():
    plan = build_plan(MANIFEST_PATH)

    assert plan["network_used"] is False
    assert plan["case_count"] == 21
    assert plan["request_count"] == 27
    assert plan["ground_truth_count"] == 124
    assert plan["decision_count"] == 164
    assert plan["neutralization_status_counts"] == {
        "applied": 6,
        "skipped": 12,
        "not_applicable": 3,
    }
...
def test_dynamic_dispatch_challenge_builds_single_blind_request():
    plan = build_plan(CHALLENGE_PATH)
    batches = prepare_batches(CHALLENGE_PATH)

    assert plan["network_used"] is False
    assert plan["case_count"] == 1
    assert plan["ground_truth_count"] == 4
    ...
    assert "handler_a" in json.dumps(batches[0].payload)
    assert "dynamic-dispatch-second-opinion" not in json.dumps(batches[0].payload)
...
def test_neutralized_arm_removes_obvious_identifier_label_signals():
    original, neutralized = prepare_batches(MANIFEST_PATH, {"basic-unused-symbols"})
...
def test_golden_manifest_requires_frozen_closed_ground_truth(tmp_path, kwargs, message):
...
def test_live_runner_uses_injected_transport_and_never_persists_key():
    ...
    def fake_transport(payload, api_key):
        ...
        return {
            "model": JEV_MODEL,
            "answers": {question_id: _choice_answer() for question_id in question_ids},
            "usage": {"input_tokens": 100, "output_tokens": 10},
        }
    ...
    assert report["latency"]["local_contract_validation_seconds"]["p95"] is not None
