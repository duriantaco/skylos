#!/usr/bin/env python3
from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parent.parent
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from skylos.benchmarks.dead_code import (  # noqa: E402
    format_summary,
    run_external_targets,
    run_manifest,
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the Skylos deterministic dead-code benchmark suite."
    )
    parser.add_argument(
        "--manifest",
        default=str(REPO_ROOT / "benchmarks" / "dead_code" / "manifest.json"),
        help="Path to the internal dead-code benchmark manifest JSON file.",
    )
    parser.add_argument(
        "--external-targets",
        default=str(REPO_ROOT / "benchmarks" / "dead_code" / "external_targets.json"),
        help="Path to the external dead-code target config JSON file.",
    )
    parser.add_argument(
        "--case",
        action="append",
        default=[],
        help="Run only the specified internal benchmark case id. Repeat for multiple ids.",
    )
    parser.add_argument(
        "--target",
        action="append",
        default=[],
        help="Run only external target id or path. Repeat for multiple targets.",
    )
    parser.add_argument(
        "--scanner",
        choices=("skylos", "vulture", "ruff"),
        default="skylos",
        help="Dead-code scanner to score against the same benchmark labels.",
    )
    parser.add_argument(
        "--strict-labels",
        action="store_true",
        help="Treat any scanner finding outside explicit unused/used labels as a false positive.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Print machine-readable JSON instead of the text summary.",
    )
    args = parser.parse_args()

    if args.target:
        summary = run_external_targets(
            args.external_targets,
            selected_targets=set(args.target),
            scanner=args.scanner,
            strict_labels=args.strict_labels,
        )
    else:
        summary = run_manifest(
            args.manifest,
            selected_cases=set(args.case),
            scanner=args.scanner,
            strict_labels=args.strict_labels,
        )

    if args.json:
        print(json.dumps(summary, indent=2))
    else:
        print(format_summary(summary))
    return 1 if summary["failure_count"] else 0


if __name__ == "__main__":
    raise SystemExit(main())
